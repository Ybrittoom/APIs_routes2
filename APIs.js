...
app.get('/alunos', (req, res) => {
    ...
    while ( index < alunos.length ) {
        if ( alunos[index].aluno === 'Ygor') {
            console.log(`Encontramos o aluno: ${alunos[index].aluno}`)
            return res.send("Encontramos o Ygor")
        } 

        index++;
    }

    res.send("Nao achamos o Ygor!!!!!")

})

app.post('/alunos', (req, res) => {
    res.send(`o ${alunos[index].aluno} sumiu novamente,MEU DEUS QUE`)
})
